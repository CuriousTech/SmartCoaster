...
//  ws=new WebSocket("ws://192.168.31.40/ws")
  date = new Date()
  ws.onopen=function(evt){setVar('tzo', date.getTimezoneOffset());setVar('hist',0)}
  ...
  switch(d.cmd)
  {
    ...
    case 'state':
      ...
      if(floz<0) floz=0
      a.ctr.innerHTML=floz+' fl.oz. &nbsp; '+oz+' oz &nbsp; '+g.toFixed(1)+' g &nbsp; '
...
<tr>
<td>Display Rate<input id='DRATE' type=text size=4 value='500' onchange="{setDRate()}"> ms</td>
<td><input type="submit" value="File Manager" onClick="window.location='/fm.html';"></td>
</tr>
<tr>
<td colspan=2>
...
function delfile(idx,name)
{
  ...
  m=document.getElementById('tbody'+idx)
  a.fileList.removeChild(m)
}
